...
def load_unet_weights2(unet, root, verbose=False):
    ...
    return unet
unet = torch.load("model-expn2-6-0.4968355009300812.pth")
preproc_fn = smp.encoders.get_preprocessing_fn("resnet34")
train_dataset = SegDataset(
    r"E:\liver2\liver2\train-75",
...
for epoch in range(epochs):
    ...
    if valid_logs['dice']>accv:
        ...
        torch.save(unet, "model-expn2-"+str(epoch)+"-"+str(accv)+".pth")

    with open('exp-n2-train.txt', 'wb') as f:
        pickle.dump(train_record, f)
'''
print("VALIDATING...")
valid_logs = valid_epoch.run(valid_loader)
valid_record.append(valid_logs)
with open('exp-n2-valid.txt', 'wb') as f:
