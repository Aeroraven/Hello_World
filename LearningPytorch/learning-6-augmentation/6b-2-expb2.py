...
def pet_augmentation():
    transform_list = [
        ...
        # albu.GaussianBlur(p=0.5, blur_limit=5),
        albu.MultiplicativeNoise(p=0.7,multiplier=(0.9, 0.95),elementwise=True)
...
unet = torch.load("model-expb2-25-0.9241566003480038.pth")
...
for epoch in range(0,epochs):
    ...
    if valid_logs['dice'] > accv:
        ...
        torch.save(unet, "model-expb2-" + str(epoch) + "-" + str(accv) + ".pth")

    with open('exp-b2-train.txt', 'wb') as f:
...
valid_record.append(valid_logs)
with open('exp-b2-valid.txt', 'wb') as f:
