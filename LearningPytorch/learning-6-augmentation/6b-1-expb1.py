...
def pet_augmentation():
    transform_list = [
        ...
        albu.GaussianBlur(p=0.5, blur_limit=7),
        albu.MultiplicativeNoise(p=0.8, multiplier=(0.85,0.9), elementwise=True)
...
unet=torch.load("model-expb1-49-0.8772350355911583.pth")
...
for epoch in range(epochs):
    ...
    if valid_logs['dice']>accv:
        accv=valid_logs['dice']
        torch.save(unet, "model-expb1-"+str(epoch)+"-"+str(accv)+".pth")

    with open('exp-b1-train.txt', 'wb') as f:
        pickle.dump(train_record, f)
'''
print("VALIDATING...")
valid_logs = valid_epoch.run(valid_loader)
valid_record.append(valid_logs)
with open('exp-b1-valid-x.txt', 'wb') as f:
